...
// ── Machine identity ─────────────────────────────────────────────────────────
// How many local IP addresses an agent may advertise in agent:register, and how
// long one of those strings may be. 45 is the longest an IP literal gets WITHOUT
// a zone index — the IPv4-mapped IPv6 form, "0000:0000:0000:0000:0000:ffff:255.255.255.255".
// A zone index ("%eth0") would push past that, and deliberately does not fit: the
// relay's sanitizer rejects "%" outright (see worker/src/local-ips.ts), because a
// zone only ever appears on a link-local address, which the agent already refuses
// to report as it identifies no machine.
//
// Alongside them, the longest hostname the relay will repeat: 253 is the maximum
// length of a fully qualified DNS name, so no real machine name is ever lost to
// the bound.
//
// All three are shared by the agent that collects the values and the relay that
// sanitizes what arrived, so the two cannot disagree about what is acceptable:
// this metadata exists to tell one machine from another in a fleet listing, not
// to inventory every adapter, and an agent must not be able to push arbitrary
// text (or an unbounded array) into a surface a model reads as prose.
export const MAX_LOCAL_IPS = 8;
export const MAX_LOCAL_IP_LENGTH = 45;
...
/**
 * WHY a job ended up with status `unknown` — the shape of its ending, recorded by
 * the agent AT THE MOMENT IT SETTLED the job, never derived later.
 *
 * `unknown` stays `unknown`: none of these values is a status, none of them says
 * the job succeeded, and none may ever be rendered as if it did. What they answer
 * is the question a real incident could not: two jobs on the same Windows box both
 * read `unknown`, one having printed nothing at all and one having printed 180 KB
 * before it died, and nothing on the record told them apart.
 *
 * EVERY TOKEN NAMES WHAT WAS OBSERVED, NEVER A CAUSE INFERRED FROM IT. The first
 * draft of this vocabulary called the log-shape endings `killed_mid_run_*`, and
 * "killed" is a diagnosis the agent has no standing to make: a command that
 * replaces the shell with `exec`, or installs an EXIT trap over the wrapper's, or
 * uses `exit` on cmd, ends NORMALLY and leaves EXACTLY this evidence whenever the
 * agent was not there to watch it — and the agent being down is routine, since
 * self-update restarts it at an arbitrary moment. buildJobScript states the same
 * contract from the wrapper's side ("still reports `unknown`, which is the honest
 * answer: nothing observed its exit code"), and this list must not contradict it.
 *
 * The values, and what the agent has to observe to write each one:
 *  - `cancelled` — someone asked for this job to stop, by id. Known from the
 *    agent's OWN action rather than from evidence, and durable: signalJob marks the
 *    record before it signals, so an agent restart inside the kill window cannot
 *    lose it. Takes precedence over every observation below.
 *  - `killed_by_signal` — the agent was running, WATCHED the job's shell exit, and
 *    the exit arrived as a SIGNAL rather than a code. The only token here that
 *    supports the SIGKILL / OOM-killer story, precisely because it is the only one
 *    where something of ours saw the ending happen.
 *  - `job_script_removed` — the same token JOB_SCRIPT_REMOVED_ERROR names, and the
 *    same story, at the other end of the job's life: the per-job `.cmd` files were
 *    gone from its directory. That check exists only at START today (it refuses the
 *    spawn); removal AFTER the spawn was invisible until this field.
 *  - `vanished_no_output_windows` / `vanished_no_output_posix` — nothing of ours
 *    observed the ending; the scripts are there and the log is EMPTY, so the job
 *    never wrote a byte.
 *  - `vanished_with_output_windows` / `vanished_with_output_posix` — nothing of
 *    ours observed the ending; the scripts are there and the log is non-empty, so
 *    the job ran and produced output before the process was gone with no exit
 *    marker. `vanished` is the whole claim: what ENDED it is not in evidence.
 *  - `undetermined` — the agent could not tell (its own stats failed). Explicitly a
 *    value rather than an absence, so "we looked and could not say" is never
 *    confused with "an old agent said nothing".
 *
 * PLATFORM IS PART OF THE TWO VANISHED TOKENS because the set of endings that are
 * POSSIBLE genuinely differs and only the agent knows which machine it is: on
 * Windows a job is never reparented, so its whole process tree goes down together
 * (Task Manager, an installer, an auto-update stopping the app); on POSIX the job
 * is reparented to PID 1 and outlives the agent, so a signal or a reboot is the
 * ending to name alongside a command that simply bypassed the exit marker. The
 * relay renders the prose from the token and has no platform of its own to consult
 * at that point, so folding the platform in here is what keeps it from guessing.
 *
 * FORWARD COMPATIBILITY: a reader must treat a value it does not recognise as
 * "said nothing" — render nothing extra — never as an error. A newer agent adding
 * a shape must not cost a caller the record it is attached to.
 */
export const JOB_UNKNOWN_REASONS = [
  "cancelled",
  "killed_by_signal",
  JOB_SCRIPT_REMOVED_ERROR,
  "vanished_no_output_windows",
  "vanished_no_output_posix",
  "vanished_with_output_windows",
  "vanished_with_output_posix",
  "undetermined",
] as const;

/** One of JOB_UNKNOWN_REASONS; see it for what each value costs to write. */
export type JobUnknownReason = (typeof JOB_UNKNOWN_REASONS)[number];

/** Whether `value` is a reason THIS build knows — see the forward-compatibility note. */
export function isJobUnknownReason(value: unknown): value is JobUnknownReason {
  return typeof value === "string" && (JOB_UNKNOWN_REASONS as readonly string[]).includes(value);
}

/**
 * The longest a reason token may be on the wire. Not a producing bound — this build
 * emits only the fixed strings above — but the accepting side has to bound an
 * agent-supplied string somehow, and a token that cannot fit any plausible future
 * name is not a token.
 */
export const JOB_UNKNOWN_REASON_MAX_CHARS = 64;
