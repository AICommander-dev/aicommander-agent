import type { JobUnknownReason } from "./constants.js";
import type { ElevatedUnavailableReason } from "./elevated-availability.js";
...
export interface AgentRegisterMsg {
  ...
  hostname: string;
  /**
   * The machine's own non-loopback IP addresses, as it sees them (see the agent's
   * collectLocalIps). DESCRIPTIVE machine metadata, exactly like `hostname` and
   * `gpus` — never a capability — and it exists for one job: telling two
   * identically-named machines apart in a fleet listing ("aic-wfs-pc" vs
   * "aic-wfs-pc2" says nothing; 10.0.0.14 vs 10.0.0.51 does).
   *
   * These are LAN/VPN addresses on the machine's own interfaces. They are not
   * routable from the relay, they say nothing about how to reach the machine, and
   * DHCP can hand the same address to a different box tomorrow — so nothing may
   * treat one as an identity or as a way in.
   *
   * OMITTED ENTIRELY when the agent found none it was confident about, and by every
   * agent too old to have this field at all. Absent therefore means NOT REPORTED,
   * never "this machine has no network" — a machine that is talking to the relay
   * self-evidently has one. Capped at MAX_LOCAL_IPS entries; agents never send `[]`.
   */
  localIps?: string[];
  platform: string;
  arch: string;
  agentVersion: string;
  /**
   * The version INSTALLED ON DISK for this agent's shipping vehicle (macOS: the
   * .app bundle's CFBundleShortVersionString, Windows: the installed exe's
   * ProductVersion, Linux: what the binary at the install path reports), as
   * opposed to `agentVersion`, which is what the RUNNING process is.
   *
   * They differ exactly when an update landed but the process was never
   * restarted — the macOS pkg upgrade that swaps the bundle under a live tray is
   * the routine way to get there — and that state is otherwise invisible: the old
   * process keeps serving the relay and keeps advertising its own old version, so
   * every consumer reports a machine that "updated" days ago.
   *
   * BEST-EFFORT and OMITTED ENTIRELY when the probe could not produce a value it
   * is sure of (no bundle, unreadable file, npm install shape, probe not finished
   * yet). Absent therefore means "unknown", NEVER "same as running" — a consumer
   * may only claim a mismatch when this is present and differs.
   */
  ...
  /**
   * WHY `elevatedExec` is false, when the agent could establish a reason (see
   * ElevatedUnavailableReason). Only ever sent alongside `elevatedExec: false`;
   * a machine that can run elevated work has nothing to explain.
   *
   * Absent means "this agent did not say" — an agent too old to report one, or a
   * probe that produced nothing it was sure of — and every consumer must fall
   * back to what it said before reasons existed. It is a DIAGNOSTIC: no code path
   * may treat a benign-looking reason as permission to run elevated work.
   */
...
export interface JobSummary {
  ...
  /**
   * WHY the outcome is `unknown` — the shape of the ending, as the agent observed
   * it at the instant it settled the job. Present only on `unknown` jobs.
   *
   * It does NOT soften the status: `unknown` still means no exit code was recorded
   * and success may never be assumed. It only tells apart endings that are
   * indistinguishable today — a job that never wrote a byte, one that wrote 180 KB
   * and then vanished, one whose own scripts were taken off the disk under it, and
   * one we cancelled ourselves.
   *
   * Optional per the one-directional widening contract (see jobs-relay.ts): an
   * older agent sends nothing and the relay renders exactly what it renders today.
   * A value this build does not recognise must also render as nothing — see
   * JOB_UNKNOWN_REASONS — which is why the field is typed as the closed union for
   * convenience but never trusted to be one of its members at runtime.
   */
...
export interface DoAgentInfoMsg {
  type: "do:agent_info";
  /**
   * The machine's own name as the RELAY will repeat it — narrowed at ingest, like
   * `localIps` below (see the worker's sanitizeHostname). Absent means the agent
   * reported no usable name (too old to send one, or one that did not survive the
   * boundary), never that the machine has none.
   */
  hostname?: string;
  /** The machine's own local addresses, when its agent reported any (see AgentRegisterMsg.localIps). */
  localIps?: string[];
  platform: string;
  arch: string;
  agentVersion: string;
  /**
   * The version on disk for this agent's shipping vehicle, when the agent could
   * determine one (see AgentRegisterMsg.installedVersion). Absent means unknown,
   * not "same as `agentVersion`".
   */
  installedVersion?: string;
  connectedAt: string;
  ...
  elevatedBootId?: string;
  /**
   * Why the connected agent cannot run elevated commands (see
   * AgentRegisterMsg.elevatedUnavailableReason). Only ever present when
   * `elevatedExec` is absent, and only when the agent named a reason.
   */
  elevatedUnavailableReason?: ElevatedUnavailableReason;
...
/**
 * WHO WROTE the `error` text on a `do:*_error` frame — the provenance the worker
 * needs before it may speak in its own authoritative voice.
 *
 * Every one of those frames carries an `error` string, and on the agent-reply path
 * the Durable Object forwards the AGENT's text verbatim (agent:error →
 * do:error, and the job/file/screenshot equivalents). The worker used to tell the
 * two apart by comparing that text against a worker-owned constant, which is not a
 * provenance check at all: an agent that emits the literal string "Agent not
 * connected" then made the relay assert, in the relay's own words, that NOTHING ran
 * on the machine and the call may be re-issued as-is — for a command that did run.
 * That is precisely the re-run-a-mutating-command hazard the reconnect path warns
 * about.
 *
 * So the DO STAMPS the frames it authors itself. Only the DO can set this field (an
 * agent's frames are parsed as AgentToDoMsg and only their `error` string is ever
 * copied across), so its presence — never the words — is what licenses the worker's
 * own message and the `unavailable` classification.
 *
 * Absent means "not a relay-authored availability failure": either the text came
 * from the machine, or it is a relay error with no such claim attached (a
 * concurrent command, say). Treat absence as agent text and clamp it.
 */
export type RelayErrorCode =
  /** No agent socket was attached when the request reached the DO (the vanished-machine race). */
  | "agent_not_connected"
  /** An agent connected, but too old to understand this request; it never heard it. */
  | "agent_too_old";
...
export interface DoScreenshotErrorMsg {
  ...
  requestId: string;
  error: string;
  /** Set ONLY when the relay authored `error`; see {@link RelayErrorCode}. */
...
export interface DoAuthOkMsg {
  ...
  elevatedBootId?: string;
  /**
   * Why the target cannot run elevated commands, when its agent said (see
   * AgentRegisterMsg.elevatedUnavailableReason). The relay's runCommand path
   * reads it ONLY to choose which remedy to print in the refusal — the refusal
   * itself is decided by `elevatedCapable`/`elevatedBootId` above and by nothing
   * here, so an unrecognised or absent reason changes the words and never the
   * verdict.
   */
  elevatedUnavailableReason?: ElevatedUnavailableReason;
  /**
...
export interface SessionStatusResponse {
  ...
  agentInfo?: {
    /**
     * The machine's own name as the relay repeats it (see DoAgentInfoMsg.hostname).
     * Absent when the agent reported no usable one.
     */
    hostname?: string;
    /**
     * The machine's own local addresses, when reported (see AgentRegisterMsg.localIps).
     * Absent means the agent did not report any — not that the machine has no network.
     */
    localIps?: string[];
    platform: string;
    arch: string;
    agentVersion: string;
    /** On-disk version, when known (see AgentRegisterMsg.installedVersion). */
    installedVersion?: string;
    connectedAt: string;
    screenShare?: ScreenShareState;
