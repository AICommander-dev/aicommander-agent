...
export const MCP_TOOL_ARGUMENT_DESCRIPTIONS = {
  remote_exec: {
    ...
    elevated:
      "Run as root (macOS) / LocalSystem (Windows) via the privileged helper. Account-only; only works on mac/Windows machines with the helper installed. Most commands do NOT need this. NOT accepted together with `shell` — that combination is rejected with an error rather than silently dropped, because the elevated path runs through a signed capability that has no shell field. AN ELEVATED COMMAND'S LIFETIME IS BOUND TO THE AGENT CONNECTION THAT REQUESTED IT: the privileged helper leases every command to that connection and kills it the moment the connection drops, so a privileged process can never outlive its authorization. That makes any command which RESTARTS, REPLACES OR KILLS AI Commander itself self-defeating when run in-band — the app IS the agent, so stopping it drops the connection, and the helper then kills the very command that asked for the stop. Measured: an elevated run of the Windows updater downloaded the new version and started the installer, the installer tree-killed the app, the lease fired, and the PowerShell waiting on the installer died mid-install — the machine stayed on the old version, the update log ends mid-sentence, and the call came back `Agent disconnected during command execution`, which READS LIKE A NETWORK BLIP while the privileged work was actually cut in half. Launch that kind of work DETACHED from this connection instead: on Windows through a scheduled task (`schtasks /Run /TN \"AI Commander Update\"` starts the updater's own SYSTEM task, which the disconnect cannot touch), on macOS through `remote_job_start` (a job reparents to PID 1 and survives anything aimed at the app), on Linux through a transient systemd unit outside the agent's control group — which is exactly what `aicommander-agent self-update` already does, and for exactly this reason.",
  },
  session_status: {
    code: MACHINE_CODE_FULL,
  },
