...
//   path we execute, or an argument we pass. The ONLY value interpolated into
//   any script in this module's impure half is a terminal-services session id,
//   validated as a non-negative integer first. That property is the one to keep;
//   nothing must ever be added that turns a read value into something executed.
//
...
//       exactly THREE, they are the whole of installBlockReason, and all three
//       are admin-owned state: the "AI Commander Update" task's RUNNING state,
//       $INSTDIR's mtime, and whether the tray exe is present. The install
//       directory they are measured against is itself derived from the Relaunch
//       task's pinned action rather than guessed. Nothing else in this module is
...
export function startWindowsWatchdog(deps: WatchdogDeps): RunningWatchdog {
  ...
  async function tick(): Promise<TickResult | null> {
    ...
    if (snapshot.unverifiedTrayProcesses > 0) {
