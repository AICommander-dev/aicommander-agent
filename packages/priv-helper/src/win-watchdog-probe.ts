...
//
// MEASURED on a real installation (2026-08-08):
//   Run value:  electron.app.AI Commander = "C:\Program Files\AI Commander\AICommander.exe"
//   user data:  --user-data-dir="C:\Users\<u>\AppData\Roaming\@aicommander/desktop"
// So the app is named after electron-builder.yml's `productName` in one place and
// after package.json's `name` in the other. There is NO single "app name"
// constant that is right for both — do not introduce one.
...
function runPowerShell(script: string): Promise<ShellResult> {
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      { windowsHide: true, timeout: PROBE_TIMEOUT_MS, maxBuffer: PROBE_MAX_OUTPUT_BYTES },
      (err, stdout) => {
...
/** Take one snapshot of the machine. Never throws; failures carry a reason. */
export async function probeWindows(): Promise<ProbeResult> {
  const shell = await runPowerShell(probeScript());
