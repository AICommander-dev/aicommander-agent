...
function handshake(ep: ElevatedEndpoint, timeoutMs: number): Promise<EndpointObservation> {
  ...
  return new Promise((resolve) => {
    ...
    let settled = false;
    /**
     * Did anything at all come back down the socket?
     *
     * A boolean about OUR socket, never the peer's text (the rule the header
     * states), and it is what lets the one deadline below name which failure
     * this is: a responder that says nothing and a responder that keeps sending
     * frames which are not an answer are different states of the machine, and
     * the second is the one this check exists to catch.
     */
    let spoke = false;
    const done = (result: EndpointObservation): void => {
      if (settled) return;
      settled = true;
      // Cleared HERE and only here, because this is the single settle point
      // every path funnels through — connect failure, protocol error, refusal,
      // answer, socket error, close, and the deadline itself. A timer left
      // armed after the promise resolves is a timer holding the process (and a
      // test runner) open for the rest of the budget.
      clearTimeout(deadline);
      socket.destroy();
      resolve(result);
    };
    // ONE TOTAL DEADLINE over the whole exchange — connect, write and answer —
    // and deliberately NOT `socket.setTimeout`, which is what this used to be.
    // That is an INACTIVITY timer: every arriving byte rearms it. The frame
    // loop below skips everything that is not `hello-ok` or `error`, so a
    // process squatting on a candidate endpoint had only to emit a well-formed
    // frame of some other type on a repeating interval to keep this check open
    // forever. `runHelperDoctor` awaits the endpoint check in sequence with the
    // others, so that hung the whole report — in exactly the state the report
    // exists to describe, something that is not the helper answering on the
    // helper's endpoint. Nothing the peer does can reset this one.
    //
    // The idle timer is gone rather than kept alongside it: at the same budget
    // it can never fire sooner than the deadline, so all it added was a second
    // way to reach the same sentence.
    //
    // `unref()` for the reason every other stray timer in this package carries
    // it: the diagnostic must never be the thing keeping the process alive.
    const deadline = setTimeout(() => {
      done({ ...base, error: spoke ? "kept the connection open without answering" : "timed out" });
    }, timeoutMs);
    deadline.unref();
    socket.once("connect", () => {
