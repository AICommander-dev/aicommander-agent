 ...
 * exact (case-insensitive) key matches. PATH/HOME/USER/LOGNAME etc. are also
 * forced from the locked base below, but listed here so a caller can never even
 * shadow them before the overlay wins.
 */
...
const isDangerEnvKey = (k: string): boolean => {
  ...
  return /^LD_/.test(up) || /^DYLD_/.test(up) || ENV_DANGER_KEYS.has(up);
};
