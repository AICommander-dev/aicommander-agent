...
/**
 * The Windows SYSTEM scheduled task that runs the helper at boot; must match
 * `desktop/build/win-privhelper-task.ps1`, which registers it (and refuses to
 * unless both installed executables carry the pinned Authenticode publisher).
 *
 * Lives HERE, with the other well-known locations, because three packages now
 * ask whether it exists — the installer that registers it, the doctor that
 * reports on it, and the agent's discovery, which uses "installed but this task
 * is missing" to tell `not_registered` apart from `endpoint_unreachable`. A name
 * typed out separately in each is a silent "helper not available" waiting to
 * happen, which is the class of bug this module exists to prevent.
 */
export const WIN_HELPER_TASK_NAME = "AI Commander Privileged Helper";
...
/**
 * Name of the Windows directory the helper is installed into, a SIBLING of
 * $INSTDIR. Identical to WIN_HELPER_TASK_NAME by coincidence of naming, not by
 * derivation: they are two different things (a folder and a scheduled task) and
 * a rename of one must not silently rename the other, so each is spelled out.
 * installer.nsh's `customInstall` is the producer.
 */
export const WIN_HELPER_DIR_NAME = "AI Commander Privileged Helper";
...
export function helperInstallDir(): string | null {
  switch (platform()) {
    case "win32":
      ...
      return `${process.env["ProgramFiles"] ?? "C:\\Program Files"}\\${WIN_HELPER_DIR_NAME}`;
    case "darwin":
