...
// against the JSON shapes PowerShell produces, the log lines an operator will
// actually read, and the contract with the desktop package that produces the
// manifest.
...
describe("the manifest contract is mirrored, and pinned to its source", () => {
  // A drifted mirror is the silent-death shape: a renamed file or a bumped
  // schema reads here as "no manifest shipped", which blocks nothing, logs
  // nothing and looks exactly like a healthy machine — i.e. the whole feature
  // would be switched off with no signal anywhere. The helper is a standalone
  // SEA binary and cannot import the desktop package, so the values are copied
  ...
  it("(fixture) really read the desktop contract", () => {
    expect(contract).toContain("MANIFEST_FILENAME");
    expect(contract).toContain("MANIFEST_SCHEMA");
  });

  it("uses the same file name the producer writes", () => {
    expect(contract).toContain(`export const MANIFEST_FILENAME = "${MANIFEST_FILENAME}";`);
  });

  it("recognises the schema the producer emits", () => {
    expect(contract).toContain(`export const MANIFEST_SCHEMA = ${MANIFEST_SCHEMA};`);
  });

  it("expects the manifest where the afterPack hook puts it on Windows", () => {
    const hook = readFileSync(
      fileURLToPath(
        new URL("../../../desktop/scripts/generate-manifest.mjs", import.meta.url),
      ),
      "utf8",
    );
    // The hook writes `resources/${MANIFEST_FILENAME}` on win32/linux; the
    // watchdog joins the same relative path onto the install root and compares
    // it against the manifest's own `manifestPath` field.
    expect(hook).toContain("`resources/${MANIFEST_FILENAME}`");
    expect(MANIFEST_REL_POSIX).toBe(`resources/${MANIFEST_FILENAME}`);
  });
});
...
describe("manifestProbeLines (text only — no PowerShell runs here)", () => {
  ...
  // ── the rule itself, run over the SHARED table ─────────────────────────────
  //
  ...
  // PowerShell host runs this suite, so it is TRANSLITERATED below — worthless
  // unless the generated lines still say what the transliteration claims, which
  // is what the pin asserts. Until 2026-09-03 this copy rejected `%` while the
  // installer's copy allowed it: two readers of one manifest, disagreeing about
  // what the manifest says.
  //
  // Accepted approximations, as in the desktop suite: .NET IsNullOrWhiteSpace vs
  // JavaScript trim(), and .NET `-match` being case-insensitive (no pattern here
  // depends on case).
  ...
  /** The fragment's rule, retyped in TypeScript. Pinned by the test below. */
  function probeRule(relative: string): boolean {
    if (relative.trim() === "") return false;
    if (/[\\:]/.test(relative)) return false;
    if (relative.startsWith("/")) return false;
    if (/(^|\/)(\/|$)/.test(relative)) return false;
    if (/(^|\/)\.{1,2}(\/|$)/.test(relative)) return false;
    if (/[\u0000-\u001F\u007F<>"|?*]/.test(relative)) return false;
    return true;
  }

  it("states the rule in exactly the lines the transliteration below mirrors", () => {
  ...
  it("agrees with the declared rule, case for case, over the shared table", async () => {
    const { isManifestEntryPath } = (await import(
      new URL("../../../desktop/src/install-manifest-contract.mjs", import.meta.url).href
    )) as { isManifestEntryPath: (value: unknown) => boolean };
    const { MANIFEST_ENTRY_CASES } = (await import(
      new URL("../../../desktop/src/__tests__/manifest-entry-cases.mjs", import.meta.url).href
    )) as { MANIFEST_ENTRY_CASES: Array<[string, boolean]> };
    expect(MANIFEST_ENTRY_CASES.length).toBeGreaterThan(20);
    for (const [value, expected] of MANIFEST_ENTRY_CASES) {
      expect(isManifestEntryPath(value), `contract: ${JSON.stringify(value)}`).toBe(expected);
      expect(probeRule(value), `probe: ${JSON.stringify(value)}`).toBe(expected);
    }
  });
