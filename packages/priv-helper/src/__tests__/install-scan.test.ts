...
import { candidateInstallRoots, scanInstall, APP_INSTALL_DIR_NAMES } from "../install-scan.js";
import {
  MANIFEST_FILENAME,
  MANIFEST_REL_POSIX,
  MANIFEST_SCHEMA,
...
describe("the entry-path rule this module mirrors", () => {
   ...
   *
   * That run READS packages/desktop, which the public agent mirror does not
   * stage, so it lives in install-manifest-desktop-parity.test.ts — the one file
   * mirror-agent.mjs's MONOREPO_ONLY excludes for this reason. What stays here is
   * everything the copy can be held to on its own.
   */
...
describe("an entry that leads out of the root through a link", () => {
  ...
  // `entryRealTarget` is this package's copy of the desktop reader's resolver,
  // and the two are run over one shared filesystem table. That table lives in
  // packages/desktop, which the public agent mirror does not stage, so the run
  // is in install-manifest-desktop-parity.test.ts.
});
