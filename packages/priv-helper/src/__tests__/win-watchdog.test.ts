...
describe("installBlockReason — never act while an install is in flight", () => {
  ...
  it("has NO 'an installer process is running' signal, and cannot grow one back", () => {
    ...
    // The list has since gained `manifest`, and it is held to the same bar: the
    // install manifest is a file in %ProgramFiles%, inside the directory derived
    // from the admin-owned Relaunch task, so no non-admin can produce it either.
