...
import type { AgentToDoMsg, DoToAgentMsg, ScreenShareState, ScreenshotDisplaySelector, RemoteOperator, GpuDevice, JobRpcResult, FileRpcResult } from "@aicommander/protocol";
...
export async function runConnectionLoop(opts: ConnectionOpts): Promise<void> {
  ...
  // else), but only exec can afford to await it: JobManager.start() is
  // synchronous by contract, so it uses whatever the memoized probe has resolved
  // SO FAR. Relying on JobManager.recover() to have warmed it is not enough —
  ...
  while (!opts.signal?.aborted) {
    ...
    try {
      ...
      if (reauthRequested && opts.reauth && !opts.signal?.aborted) {
        ...
        try { token = await opts.reauth(); } catch { /* keep current token */ }
      }
    } catch (err) {
...
async function connectAndServe(
  ...
  return new Promise<boolean>((resolve, reject) => {
    ...
    // capabilities. Records what we advertised so a later bootId change can be
    // detected. No-op once the connection has settled.
    const sendRegister = (elevatedBootId: string | undefined): void => {
      if (settled) return;
      registerEpoch++;
    ...
    // discoverHelper() itself retries a few times, so a transient probe failure at
    // register time no longer wrongly disables elevated exec for the whole session.
    const reconcileElevated = async (): Promise<void> => {
      if (settled) return;
      const epoch = registerEpoch;
      const found = isElevatedHelperAvailable() ? await discoverHelper() : null;
      ...
      if (found === null) {
        ...
        if (registeredBootId !== undefined) sendRegister(undefined);
        return;
      }
    ...
    const jobs = (): JobManager => {
      ...
      // there: JobManager.start() is synchronous and must not wait on
      // `nvidia-smi`, and the probe policy (notably "a failed re-probe is not
     ...
     */
    const replyJob = (requestId: string, run: () => JobRpcResult): void => {
      let reply: AgentToDoMsg;
      try {
        reply = { type: "agent:job_result", requestId, result: run() };
      } catch (err) {
        reply = {
     ...
     * compatible with the manager's invariants: start() stays synchronous
     * (record written before spawn, so a status poll can never observe a
     * `running` job with no pid), and the delay lives in the frame handler, which
     * is already async and does not block the socket — `do:ping` keeps being
     ...
     * perfectly healthy 100 MiB upload. Job RPCs can be answered inline precisely
     * because they are disk-local and quick; this one cannot.
     *
    ...
    const doResolve = () => { if (!settled) { settled = true; clearReauth(); abortActiveCommand(); abortTransfers(); stopGpuPoll(); stopElevatedReconcile(); resolve(reauthRequested); } };
    const doReject = (err: Error) => { if (!settled) { settled = true; clearReauth(); abortActiveCommand(); abortTransfers(); stopGpuPoll(); stopElevatedReconcile(); reject(err); } };
    ...
    ws.on("message", (raw) => {
      ...
      switch (msg.type) {
        ...
        // back here, it is read afterwards with do:job_logs. All are answered
        // synchronously except do:job_start, which may first wait out the
        // login-shell PATH probe (see startJob).
        case "do:job_start":
