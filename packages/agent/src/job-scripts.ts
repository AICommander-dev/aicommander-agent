 ...
 *
 * Split out of job-manager.ts, which owns the job LIFECYCLE (records, liveness,
 * GPU locks, retention). The split is a SEPARATION OF CONCERNS and nothing more:
 * job-manager.ts is still far past the 1000-line limit and this extraction did
 * not change that (it grew, on net, with the start-window work). Bringing it
 * under the limit is a deferred decision of its own, recorded as W9.
 ...
 * execution model of the Windows pair are FROZEN. Four properties are
