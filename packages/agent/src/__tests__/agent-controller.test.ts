...
describe("AgentController", () => {
  ...
  // ── The diagnostic log: one fault, one name, on both hosts ─────
  describe("naming a local path fault", () => {
    // On the desktop host a saveSession failure — an EACCES/EPERM on our own
    // credential store, i.e. the antivirus signature — was caught by the
    // registration loop and written down as `conn.register_failed`, while the
    // headless host (run.ts) recorded the identical fault as
    // `path.error path_role=sessionStore`. On Windows, the platform this work
    // targets, that made a local permission fault read as "the relay rejected
    // registration" in one of the two files a support case can arrive with.
    let logDir: string;

    beforeEach(() => {
      logDir = fs.mkdtempSync(path.join(os.tmpdir(), "aic-ctrl-diag-"));
      initDiagLog({ dir: logDir, role: "main" });
    });
    ...
    /** The log, written by its own timer exactly as it is in production. */
    async function diagText(): Promise<string> {
      await new Promise((resolve) => setTimeout(resolve, 20));
      try {
        return fs.readFileSync(path.join(logDir, "main.log"), "utf8");
      } catch {
        return "";
      }
    }

    it("calls a refused credential store a path fault, not a registration failure", async () => {
      mockRegisterOk();
      const denied: NodeJS.ErrnoException = new Error("EACCES: permission denied, open");
      denied.code = "EACCES";
      denied.syscall = "open";
      // Once: the loop retries, and a store that never recovers would spin.
      ...
      expect(text).toContain("path.error path_role=sessionStore code=EACCES syscall=open");
      // The same event must not ALSO be reported as the relay refusing us.
      expect(text).not.toContain("conn.register_failed");
    });

    it("still records a genuine registration refusal as one", async () => {
      vi.mocked(register).mockRejectedValueOnce(
        new RegistrationError(429, '{"code":"rate_limited"}'),
      );
      mockRegisterOk();
      const ctrl = makeCtrl();
      ...
      expect(text).toContain("conn.register_failed status=429 relay_code=rate_limited");
      expect(text).not.toContain("path.error");
    });
  });
