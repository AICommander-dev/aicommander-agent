...
beforeEach(() => {
  store.clear();
  vi.clearAllMocks();
  savedService = process.env["AICOMMANDER_SERVICE"];
  delete process.env["AICOMMANDER_SERVICE"];
});

afterEach(() => {
  vi.restoreAllMocks();
  if (savedService === undefined) delete process.env["AICOMMANDER_SERVICE"];
  else process.env["AICOMMANDER_SERVICE"] = savedService;
});
