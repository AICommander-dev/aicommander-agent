...
// The on-disk version probe reads the running .app bundle (macOS) or spawns
// PowerShell / the installed binary. None of that describes the machine running
// this suite, so the module boundary is stubbed and the snapshot is driven
// explicitly by the register tests below; the probe itself is covered in
// installed-version.test.ts.
vi.mock("../installed-version.js", () => ({
  startInstalledVersionProbe: vi.fn(async () => undefined),
  installedVersionSnapshot: vi.fn(() => installedVersionForTest),
}));
let installedVersionForTest: string | undefined;
...
// The connection asks ONE question about elevated execution — "is there a helper,
// and if not, why not?" — so the stub answers exactly that. The default is the
// Linux answer (no helper by design), which is what most of these tests are: a
// machine that will never advertise elevatedExec, and now says so.
vi.mock("../elevated-availability.js", () => ({
  resolveElevatedAvailability: vi.fn(async () => ({
    available: false as const,
    reason: "platform_unsupported" as const,
  })),
  // The synchronous half the open-time register carries. Default `undefined` —
  // "nothing cheap to say" — so every test that does not care about it sees the
  // pre-existing two-frame behaviour.
  staticElevatedUnavailableReason: vi.fn((): string | undefined => undefined),
}));
...
import { closeDiagLog, flushDiagLog, initDiagLog, noteDiagnosed } from "../diag-log.js";
...
describe("WebSocket connection", () => {
  beforeEach(() => {
    ...
    // Default for every test: a machine with no privileged helper by design
    // (Linux). Tests that care about elevated exec override it.
    vi.mocked(resolveElevatedAvailability).mockResolvedValue({
      available: false,
      reason: "platform_unsupported",
    });
    // …and, by default, nothing knowable without probing, so the open-time
    // register carries no reason and the reconcile is what supplies one. The one
    // test about the cheap verdicts overrides it.
    vi.mocked(staticElevatedUnavailableReason).mockReturnValue(undefined);
  ...
  it("sends agent:register on open", async () => {
    ...
    // The machine's own addresses, so a fleet listing can tell this box from the
    // next one. Asserted by SHAPE, not by value: which addresses the machine
    // running this suite has is not a fixture. The rules that decide them live in
    // local-addresses.test.ts; what matters here is that the frame never carries
    // an empty array (absent means "not reported", `[]` would claim the machine
    // has no network) and never carries loopback.
    if (register.localIps !== undefined) {
      expect(Array.isArray(register.localIps)).toBe(true);
      expect(register.localIps.length).toBeGreaterThan(0);
      expect(register.localIps.every((ip: unknown) => typeof ip === "string")).toBe(true);
      expect(register.localIps).not.toContain("127.0.0.1");
    }
    ws.close(1000, "ok");
  });

  // ── What is on disk vs what is running ────────────────────────────────────
  describe("installedVersion", () => {
    afterEach(() => {
      installedVersionForTest = undefined;
    });

    it("advertises the probed on-disk version", async () => {
      installedVersionForTest = "1.1.0";
      const { ws } = await connect();
      ws._emit("open");
      await new Promise((r) => setTimeout(r, 0));

      const register = JSON.parse(ws.sent[0]!);
      expect(register.installedVersion).toBe("1.1.0");
      ws.close(1000, "ok");
    });

    it("OMITS the field when the probe knows nothing — absent is not 'same as running'", async () => {
      installedVersionForTest = undefined;
      const { ws } = await connect();
      ws._emit("open");
      await new Promise((r) => setTimeout(r, 0));
      ...
      const register = JSON.parse(ws.sent[0]!);
      expect("installedVersion" in register).toBe(false);
      ws.close(1000, "ok");
    });
  });
  ...
  describe("login-shell PATH warm-up", () => {
    ...
    it("hands the manager a signal for a socket that dies AFTER the start begins", async () => {
      // The `settled` check above only covers the probe waits that precede
      // start(). start() itself is asynchronous now — on Windows it writes the
      // job's two scripts and reads them back before the spawn — and a socket
      // dying in THAT window used to produce exactly the orphan this handler
      // refuses to create: a running job whose id was never delivered, holding a
      // GPU lock and a concurrency slot until the machine was restarted. A
      // boolean cannot reach work that is already parked in an await; the signal
      // can, so the manager can drop the start before it spawns.
      const { ws } = await connect();
      ws._emit("open");
      await new Promise((r) => setTimeout(r, 0));
      ...
      const manager = vi.mocked(getJobManager).mock.results.at(-1)!.value as unknown as {
        start: { mock: { calls: [{ signal?: AbortSignal }][] } };
      };
      const signal = manager.start.mock.calls[0]![0].signal;
      expect(signal).toBeInstanceOf(AbortSignal);
      // Live while the start is: aborting it any earlier would refuse jobs on a
      // healthy socket.
  ...
  it("registers immediately, then re-registers with elevatedBootId once discovery completes", async () => {
    vi.mocked(resolveElevatedAvailability).mockResolvedValueOnce({
      available: true,
      bootId: "boot-xyz",
    ...
    // A machine that CAN run elevated work has nothing to explain.
    expect(second.elevatedUnavailableReason).toBeUndefined();
    ws.close(1000, "ok");
  });

  it("re-registers with the REASON when there is no helper, and never a capability", async () => {
    // This is the 2026-09-02 machine: files present, SYSTEM task never registered.
    // The immediate register says elevatedExec:false and nothing else, so the
    // reconcile MUST send a second frame — otherwise the relay cannot tell this
    // apart from an agent too old to report one, which is the whole defect.
    vi.mocked(resolveElevatedAvailability).mockResolvedValueOnce({
      available: false,
    ...
    expect(registers[0].elevatedUnavailableReason).toBeUndefined();
    expect(registers[1].elevatedExec).toBe(false);
    expect(registers[1].elevatedBootId).toBeUndefined();
    expect(registers[1].elevatedUnavailableReason).toBe("not_registered");
    ws.close(1000, "ok");
  });

  it("does not re-register while the reason stays the same", async () => {
    // The reconciler runs every 60s forever. A reason that has not changed is not
    // news, and a register frame per minute per helper-less machine would be.
    vi.mocked(resolveElevatedAvailability).mockResolvedValue({
      available: false,
    ...
    ws._emit("open");
    await vi.advanceTimersByTimeAsync(0);
    // One more turn of the 60s reconciler, kept under the 90s ping timeout so the
    ...
    const registers = ws.sent
      ...
      .filter((m) => m.type === "agent:register");
    expect(registers).toHaveLength(2);
    expect(registers[1].elevatedUnavailableReason).toBe("not_installed");
    ws.close(1000, "ok");
  });

  it("sends ONE register on a machine whose reason was knowable without probing", async () => {
    // Every Linux box, and every mac/Windows box with no helper installed. The
    // open-time frame used to carry no reason at all, so the first reconcile
    // always found the reason "new" and sent a SECOND agent:register — on every
    // connection of every such machine in the fleet, and each register costs the
    // relay two storage writes, a pending-register clear, a broadcast to every
    // dashboard and a ping. Both cheap verdicts are final, so the first frame is
    // already the right one.
    vi.mocked(staticElevatedUnavailableReason).mockReturnValue("platform_unsupported");
    vi.mocked(resolveElevatedAvailability).mockResolvedValue({
      available: false,
      reason: "platform_unsupported",
    });
    ...
    ws._emit("open");
    await vi.advanceTimersByTimeAsync(0);
    // A full reconcile turn, kept under the 90s ping timeout.
    await vi.advanceTimersByTimeAsync(61_000);

    const registers = ws.sent.map((s) => JSON.parse(s)).filter((m) => m.type === "agent:register");
    expect(registers).toHaveLength(1);
    expect(registers[0].elevatedExec).toBe(false);
    expect(registers[0].elevatedUnavailableReason).toBe("platform_unsupported");
    ws.close(1000, "ok");
  });

  it("retracts the capability AND says why when a live helper stops answering", async () => {
    // The path a helper that is quarantined, crashed or stopped mid-session
    // takes, and the only one that has to do two things at once: stop the relay
    // minting capabilities the helper would reject (elevatedExec:false), and say
    // what happened, or the machine goes back to being indistinguishable from an
    // old agent and an offline box — the defect this work exists to remove.
    vi.mocked(resolveElevatedAvailability)
      .mockResolvedValueOnce({
        available: true,
        bootId: "boot-live",
        endpoint: { transport: "tcp", host: "127.0.0.1", port: 42847 },
      })
      .mockResolvedValue({ available: false, reason: "endpoint_unreachable" });
    ...
    // The helper was live and advertised…
    expect(registers[1].elevatedExec).toBe(true);
    expect(registers[1].elevatedBootId).toBe("boot-live");
    // …and the retraction takes the capability away and names the cause in the
    // same frame. A retraction with no reason would be the original defect.
    expect(registers[2].elevatedExec).toBe(false);
    expect(registers[2].elevatedBootId).toBeUndefined();
    expect(registers[2].elevatedUnavailableReason).toBe("endpoint_unreachable");
    ws.close(1000, "ok");
  });
  ...
  it("discards a stale discovery result when a fresher bootId was registered meanwhile", async () => {
    ...
    let resolveDiscovery:
      | ((v: {
          available: true;
          bootId: string;
          endpoint: { transport: "tcp"; host: string; port: number };
        }) => void)
      | undefined;
    vi.mocked(resolveElevatedAvailability).mockImplementationOnce(
  ...
  // ── One fault, one name ───────────────────────────────────────────────────
  describe("a token rotation that failed on our own credential store", () => {
    /** Drive one idle rotation whose reauth throws, and hand back the log text. */
    async function rotateAndFail(err: unknown): Promise<string> {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aic-conn-diag-"));
      ...
      await flushTicketHandshake();
      latestWS()._emit("open"); // arms the reauth timer (idle)
      await vi.advanceTimersByTimeAsync(61_000);
      ac.abort();
      vi.useRealTimers();
      await flushDiagLog();
    ...
    it("keeps the precise name the raiser gave it instead of adding a second one", async () => {
      // The rotation re-saves the session, so an EACCES here is our own store
      // refusing us — the antivirus signature, already recorded at the point it
      // happened as `path.error path_role=sessionStore` (agent-controller.ts).
      // Naming it AGAIN here made one fault read as two events in the file a
      // support engineer is given, which is exactly the duplication the
      // diagnosed-locally mark exists to prevent. The mark used to be written and
      // never read on this path: the error never reaches the controller's catch,
      // because a failed rotation is swallowed here.
      const storeFault = Object.assign(new Error("permission denied"), {
        code: "EACCES",
        syscall: "open",
      });
      noteDiagnosed(storeFault);

      expect(await rotateAndFail(storeFault)).not.toContain("conn.token_rotate_failed");
    });

    it("still names a rotation nobody else named", async () => {
      // The other half: the relay refusing the re-register, a network fault. No
      // one has recorded it, so silence here would lose it entirely.
      const text = await rotateAndFail(new Error("relay said no"));

      expect(text).toContain("conn.token_rotate_failed");
    });
  });
