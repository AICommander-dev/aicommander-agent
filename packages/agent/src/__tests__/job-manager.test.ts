...
vi.mock("../atomic-file.js", async () => {
  ...
  return {
    ...
    // BOTH WRITERS, and the async one is not optional. The Windows job scripts
    // moved to `atomicWriteUtf8Async` so the write cannot block Electron's main
    // loop; a hook installed only on the sync twin then stopped firing for
    // `command.cmd`, and the Windows-only staging below silently did nothing.
    // CI caught it because the staging is asserted (`expect(removals).toBe(1)`)
    // rather than assumed — a mock that quietly stops matching its target is
    // indistinguishable from a passing test otherwise.
    atomicWriteUtf8Async: async (
      dir: string,
      fileName: string,
      contents: string,
      giveUp?: Parameters<typeof actual.atomicWriteUtf8Async>[3],
    ): Promise<void> => {
      metaWrite.before?.(dir, fileName);
      if (metaWrite.fail?.(dir, fileName)) throw new Error("read-only file system");
      await actual.atomicWriteUtf8Async(dir, fileName, contents, giveUp);
    },
  };
});
...
async function startJob(req: Parameters<JobManager["start"]>[0]): Promise<JobSummary> {
  return expectJob(await manager.start(req));
}
...
describe("resolveJobsRoot", () => {
  it("puts jobs under an explicitly supplied config directory", async () => {
    expect(resolveJobsRoot("/opt/aic-data")).toBe(path.join("/opt/aic-data", "jobs"));
  });

  it("honours AICOMMANDER_CONFIG_DIR when no config directory is passed", async () => {
    const durable = path.join(tmpBase, "durable");
    process.env[ENV_DIR_VAR] = durable;
    expect(resolveJobsRoot()).toBe(path.join(durable, "jobs"));
  });

  it("returns an absolute per-machine path with no override at all", async () => {
...
describe("getJobManager", () => {
  it("returns the same instance for the same root and a new one after reset", async () => {
  ...
  it("recovers the jobs root on first use", async () => {
...
describeOnPosix("job lifecycle", () => {
  it("runs a job to completion and reports exit code 0", async () => {
    const started = await startJob({ command: "printf 'lifecycle-output'" });
  ...
  it("reports a non-zero exit code", async () => {
    ...
    const started = await startJob({ command: "printf 'failing' >&2; (exit 7)" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("exited");
  ...
  it("interleaves stdout and stderr into one output.log", async () => {
    const started = await startJob({ command: "printf 'to-out'; printf 'to-err' >&2" });
    await waitForExit(started.jobId);
  ...
  it("runs in a per-job workspace when no cwd was given", async () => {
    const started = await startJob({ command: "pwd" });
    await waitForExit(started.jobId);
  ...
  it("runs in a caller-supplied cwd", async () => {
    ...
    const started = await startJob({ command: "pwd", cwd: elsewhere });
    await waitForExit(started.jobId);
  ...
  it("lists jobs newest first and filters by status", async () => {
    const finished = await startJob({ command: "printf done" });
    await waitForExit(finished.jobId);
    const running = await startJob({ command: LONG_RUNNING });
  ...
  it("reports logBytes so a caller can page without a probe read", async () => {
    const started = await startJob({ command: "printf '0123456789'" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
  ...
  it("normalizes the job name and falls back to a stable default", async () => {
    ...
    const named = await startJob({ command: "printf x", name: "\ttrain\nrun " });
    expect(named.name).toBe("train run");

    const long = await startJob({ command: "printf x", name: "n".repeat(200) });
    expect(long.name).toHaveLength(64);

    const unnamed = await startJob({ command: "printf x" });
    expect(unnamed.name).toBe(`job-${unnamed.jobId.slice(0, 8)}`);
  });

  it("refuses a request that is malformed IN ITSELF with invalid_request", async () => {
    ...
    for (const [req, message] of cases) {
      const refusal = expectRefusal(await manager.start(req));
  ...
  it("keeps a cwd that is missing on THIS machine a machine fault, not a caller fault", async () => {
    ...
    await expect(
      manager.start({ command: "printf x", cwd: path.join(tmpBase, "nope") }),
...
describeOnPosix("job HOME", () => {
  itUnlessRoot("leaves HOME alone for a non-root agent, whatever cwd says", async () => {
    ...
    fs.mkdirSync(elsewhere);
    const started = await startJob({ command: 'printf %s "$HOME"', cwd: elsewhere });
    await waitForExit(started.jobId);
  ...
  it("lets an explicit caller HOME win", async () => {
    ...
    fs.mkdirSync(chosen);
    const started = await startJob({ command: 'printf %s "$HOME"', env: { HOME: chosen } });
    await waitForExit(started.jobId);
  ...
  it("keeps the exit-marker path out of a caller's reach", async () => {
    ...
    const started = await startJob({ command: "exit 2", env: { [JOB_EXIT_PATH_ENV]: decoy } });
    await waitForExit(started.jobId);
...
describeOnPosix("restart recovery", () => {
  it("(a) settles a job from its exit file when the agent was down", async () => {
  ...
  it("(a) accepts a negative exit code and ignores a garbage exit file", async () => {
  ...
  it("(b) adopts a job whose pid is still alive as running", async () => {
    const started = await startJob({ command: LONG_RUNNING });
  ...
  it("(c) reports a job with no exit file and a dead pid as unknown, never as success", async () => {
  ...
  it("(b) rejects a recycled pid via the recorded process start time", async () => {
  ...
  it("(b) holds a live pid it cannot corroborate instead of declaring it finished", async () => {
  ...
  it("(b) holds a live pid whose recorded identity is a LEGACY token, keeping its GPU lock", async () => {
  ...
  it("(b) holds a live pid whose recorded identity is a locally-parsed epoch token, keeping its GPU lock", async () => {
  ...
  it("deletes job directories older than the retention window and keeps recent ones", async () => {
    const stale = seedJob({
      status: "exited",
      exitCode: 0,
  ...
  it("keeps an unreadable job directory until it is older than the retention window", async () => {
  ...
  it("ignores directories and files that are not job ids", async () => {
  ...
  it("never throws when the jobs root is unusable", async () => {
...
describe("retention", () => {
  ...
  it("reclaims expired jobs while it is running, not only at startup", async () => {
  ...
  it("reclaims expired jobs when the next job starts", async () => {
    const live = new JobManager({ jobsRoot });
    live.recover();
    const stale = seedJob({ status: "exited", exitCode: 0, endedAt: Date.now() });

    ageTheAgentBy(PAST_RETENTION);
    expectJob(await live.start({ command: "printf x" }));

    expect(fs.existsSync(jobDir(stale))).toBe(false);
  });

  it("keeps a job that ended but has not been settled yet", async () => {
  ...
  it("does not scan the jobs root at all on the polling calls", async () => {
...
describeOnPosix("cost of a full jobs root", () => {
  it("refreshes at most the page it returns", async () => {
  ...
  it("refreshes at most the DEFAULT page when the caller named no limit", async () => {
  ...
  it("does not re-read finished jobs on every start once a walk has covered them", async () => {
    for (let i = 0; i < 50; i++) {
    ...
    await startJob({ command: "printf x" });
  ...
  it("re-opens exactly the records the default page left unsettled", async () => {
    ...
    const seedsReadBy = async (act: () => Promise<void>): Promise<string[]> => {
      const readFileSync = vi.spyOn(fs, "readFileSync");
      try {
        await act();
    ...
    expect(await seedsReadBy(async () => void (await startJob({ command: "printf x" })))).toEqual(
      [...unsettled].sort(),
    );
    ...
    const restarted = new JobManager({ jobsRoot });
    const afterRestart = await seedsReadBy(async () => {
      expectJob(await restarted.start({ command: "printf x" }));
    });
...
describeOnPosix("exit marker", () => {
  ...
  it("reports the exit code of a job that exits the shell", async () => {
    const started = await startJob({ command: "printf 'before exit'; exit 5" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("exited");
  ...
  it("reports the exit code of a set -e job that fails", async () => {
    const started = await startJob({ command: "set -e\nfalse\nprintf 'never runs'" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("exited");
  ...
  it("reports the exit code of a job that replaces the shell with exec", async () => {
    ...
    const started = await startJob({ command: "exec sh -c 'printf execed; exit 4'" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("exited");
  ...
  it("reports the exit code of a job that installs its own EXIT trap", async () => {
    const started = await startJob({ command: "trap 'printf cleanup' EXIT; exit 6" });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
  ...
  it("never overwrites an exit code that is already recorded", async () => {
    ...
    const started = await startJob({ command: "exec sh -c 'sleep 0.4; exit 4'" });
  ...
  it("still accepts a multi-line command on POSIX", async () => {
    ...
    const started = await startJob({ command: "printf first\nprintf second" });
    await waitForExit(started.jobId);
  ...
  it("keeps the missing-cwd branch terminal in the Windows command script", async () => {
  ...
  it("captures a job's output without the caller ever asking for it", async () => {
    ...
    const started = await startJob({ command: "printf 'hello from the job'; printf oops 1>&2" });
    await waitForExit(started.jobId);
...
describeOnWindows("Windows job capture", () => {
  it("runs a job through the real spawn path and captures its output", async () => {
    const started = await startJob({ command: "node -e \"console.log('hello-from-the-job')\"" });
    await waitForExit(started.jobId);
  ...
  it("captures stderr and the command's own exit code", async () => {
    ...
    const started = await startJob({ command: "echo to-stderr 1>&2 & exit /b 3" });
    await waitForExit(started.jobId);
  ...
  it("captures the output of processes the job starts", async () => {
    ...
    const started = await startJob({ command: 'cmd /d /s /c "echo from-a-child"' });
    await waitForExit(started.jobId);
  ...
  it("delivers a quoted argument holding `|` and `&` as one argument", async () => {
    ...
    const control = await startJob({ command: 'node -e "console.log(process.argv[1])" "a b c"' });
    ...
    const started = await startJob({ command: 'node -e "console.log(process.argv[1])" "a | b & c"' });
    await waitForExit(started.jobId);
  ...
  it("propagates the exit code of a child that never exits the shell itself", async () => {
    ...
    const started = await startJob({ command: 'node -e "process.exit(7)"' });
    await waitForExit(started.jobId);
  ...
  it("runs a command ending in a caret without swallowing the script's own tail", async () => {
    ...
    const started = await startJob({ command: "node -e \"console.log('caret-tail')\" ^" });
    await waitForExit(started.jobId);
  ...
  it("refuses a command containing a line break", async () => {
    ...
    const refusal = expectRefusal(await manager.start({ command: "echo one\r\necho two" }));
    expect(refusal.reason).toBe("invalid_request");
    expect(expectRefusal(await manager.start({ command: "echo one\necho two" })).reason).toBe(
      "invalid_request",
    );
    expect(expectRefusal(await manager.start({ command: "echo one\recho two" })).reason).toBe(
      "invalid_request",
    );
  ...
  it("does not run the command when the cwd vanished before the spawn", async () => {
    ...
    const control = await startJob({ command, cwd: vanishing });
    await waitForExit(control.jobId);
    expect(expectJob(manager.status({ jobId: control.jobId })).exitCode).toBe(0);
    ...
    const started = await startJob({ command, cwd: vanishing });
    await waitForExit(started.jobId);
  ...
  it("captures a job whose jobs root contains a percent pair", async () => {
    ...
    const started = expectJob(await pctManager.start({ command: "node -e \"console.log('pct-ok')\"" }));
...
describeOnPosix("a start that cannot own its process", () => {
  ...
  it("keeps the card reserved when the process identity cannot be captured", async () => {
    ...
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 7 }));
  ...
  it("holds the record and the card until a process it could not record is gone", async () => {
    ...
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 8 }));
    expect(refusal.reason).toBe("gpu_busy");
    await expectStillRunning();
  ...
  it("keeps a mid-retirement process's card across an agent RESTART", async () => {
    ...
    const refusal = expectRefusal(await restarted.start({ command: "printf second", gpuIndex: 11 }));
    expect(refusal.reason).toBe("gpu_busy");
    await expectStillRunning();
  ...
  it("still starts normally when the process ended before we could write it down", async () => {
    ...
    const started = await startJob({ command: "printf quick", gpuIndex: 9 });
    await waitForExit(started.jobId);
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("exited");
    expect(settled.exitCode).toBe(0);
...
describeOnPosix("a live process we cannot vouch for", () => {
  ...
  it("holds a running job whose identity probe fails transiently", async () => {
    ...
    const started = await startJob({ command: TICKER, gpuIndex: 10, env: { AIC_JOB_TICK: tickFile } });
    await waitFor("the ticker to start writing", () => fileSize(tickFile) > 0);
    ...
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 10 }));
    expect(refusal.reason).toBe("gpu_busy");
...
describe("liveness-probe errno routing", () => {
  ...
  it("EPERM on a pid whose identity still matches holds the job and its lock", async () => {
    ...
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: GPU }));
    expect(refusal.reason).toBe("gpu_busy");
...
describeOnPosix("GPU lock", () => {
  it("reserves the card and exposes it to the job as CUDA_VISIBLE_DEVICES", async () => {
    const started = await startJob({ command: 'printf %s "$CUDA_VISIBLE_DEVICES"', gpuIndex: 2 });
  ...
  it("refuses a second job on the same card with gpu_busy and the holder's id", async () => {
    const holder = await startJob({ command: LONG_RUNNING, gpuIndex: 0 });
    const before = fs.readdirSync(jobsRoot).length;

    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 0 }));
    expect(refusal.reason).toBe("gpu_busy");
  ...
  it("allows a different card while one is held", async () => {
    await startJob({ command: LONG_RUNNING, gpuIndex: 0 });
    const other = await startJob({ command: LONG_RUNNING, gpuIndex: 1 });
  ...
  it("releases the lock as soon as the job is observed to have ended", async () => {
    const started = await startJob({ command: "printf done", gpuIndex: 3 });
  ...
  it("frees the card on cancel, without waiting for anyone to poll the job", async () => {
    ...
    const started = await startJob({ command: LONG_RUNNING, gpuIndex: 2 });
    ...
    expect((await startJob({ command: LONG_RUNNING, gpuIndex: 2 })).gpuIndex).toBe(2);
  });

  it("reaps a stale lock during startup recovery but keeps a live holder's", async () => {
    const live = await startJob({ command: LONG_RUNNING, gpuIndex: 0 });
  ...
  it("reaps a stale lock on acquisition so one crash cannot wedge a card", async () => {
    ...
    const started = await startJob({ command: LONG_RUNNING, gpuIndex: 4 });
  ...
  it("never steals a lock held by another job when settling", async () => {
    const first = await startJob({ command: "printf done", gpuIndex: 5 });
  ...
  it("refuses an implausible gpuIndex as invalid_request, never dropping the field", async () => {
    ...
    for (const gpuIndex of [-1, 1.5, 4_096, Number.NaN, Number.POSITIVE_INFINITY]) {
      const refusal = expectRefusal(await manager.start({ command: "printf x", gpuIndex }));
      expect(refusal.reason, `gpuIndex=${gpuIndex}`).toBe("invalid_request");
      expect(refusal.message).toMatch(/gpu_index/);
    }
    expect(
      expectRefusal(await manager.start({ command: "printf x", gpuIndex: "0" as unknown as number }))
        .reason,
...
describeOnPosix("GPU lock reaping when the disk will not answer", () => {
  ...
  it("answers gpu_busy instead of stealing the lock when the holder's record cannot be read", async () => {
    ...
    failReadsOf(path.join(holder, "meta.json"));
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 14 }));
  ...
  it("holds a lock FILE that itself cannot be read, on recovery and on acquisition", async () => {
    ...
    const refusal = expectRefusal(await manager.start({ command: "printf second", gpuIndex: 16 }));
    expect(refusal.reason).toBe("gpu_busy");
    expect(fs.existsSync(gpuLockPath(16))).toBe(true);
  });
});
...
describe("log slicing", () => {
  ...
  it("lets maxBytes only shrink the slice", async () => {
  ...
  it("clamps an offset past the end to the end and reports eof", async () => {
  ...
  it("returns an empty slice for a job that produced no output at all", async () => {
    const jobId = seedJob({ status: "exited", exitCode: 0, endedAt: Date.now() });
    const logs = expectLogs(manager.logs({ jobId }));
    expect(logs.chunk).toBe("");
    expect(logs.offsetBytes).toBe(0);
  ...
  it("follows a growing log across calls", async () => {
...
describeOnPosix("log cap", () => {
  ...
  it("cannot stop the WRITER, so the file really does outgrow the cap", async () => {
    ...
    const started = await startJob({ command: "while :; do printf 'chatty-output'; sleep 0.02; done" });
    await waitFor("the job's log file to exist", () => fs.existsSync(logPath(started.jobId)));
    inflateLog(started.jobId, JOB_MAX_LOG_BYTES);
  ...
  it("caps a job that exits before it is ever polled", async () => {
    ...
    const started = await startJob({ command: GATED });
    await waitFor("the job's log file to exist", () => fs.existsSync(logPath(started.jobId)));
  ...
  it("caps a job that was already terminal on disk when this agent found it", async () => {
...
describeOnPosix("concurrency limit", () => {
  // 32 real spawns plus the per-start walk of everything already on disk; see
  // the sibling case in "refusal and payload discipline" for the measurement.
  it("refuses a start beyond JOB_MAX_CONCURRENT running jobs", async () => {
    for (let i = 0; i < JOB_MAX_CONCURRENT; i++) {
      await startJob({ command: LONG_RUNNING });
    }
    const before = fs.readdirSync(jobsRoot).length;

    const refusal = expectRefusal(await manager.start({ command: "printf over-the-limit" }));
  ...
  it("counts from live state, not from stale 'running' records on disk", async () => {
    ...
    const started = await startJob({ command: "printf accepted" });
    expect(started.status).toBe("running");
  });
});
...
describeOnPosix("cancel", () => {
  ...
  async function startTicker(tickFile: string, trapTerm: boolean): Promise<JobSummary> {
    ...
    const command = trapTerm
      ...
      : `${loop} & printf 'ready\\n'; wait`;
    const started = await startJob({ command, env: { AIC_JOB_TICK: tickFile } });
    await waitFor("the ticker to start writing", () => fileSize(tickFile) > 0);
    return started;
  }

  it("refuses to cancel a job that does not exist", async () => {
  ...
  it("cancelling an already-finished job returns its terminal state, not an error", async () => {
    const started = await startJob({ command: "printf done" });
    await waitForExit(started.jobId);
  ...
  it("never reports a cancelled job as 'exited 0' when its shell leaves cleanly", async () => {
    ...
    const started = await startJob({ command: "trap 'exit 0' TERM; while :; do sleep 0.05; done" });
  ...
  it("still keeps a real exit code the wrapper recorded before the cancel landed", async () => {
    ...
    const started = await startJob({ command: "printf done; exit 7" });
    await waitForExit(started.jobId);
  ...
  it("records CANCELLED as the reason, so a killed job never reads as a machine fault", async () => {
    // Same shell as the test above: it handles SIGTERM and leaves cleanly, so an
    // exit marker EXISTS and is refused. The status is `unknown`, and the reason
    // says which kind of unknown — the one with a known cause.
    const started = await startJob({ command: "trap 'exit 0' TERM; while :; do sleep 0.05; done" });
    await waitFor(
    ...
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(settled.status).toBe("unknown");
    expect(settled.unknownReason).toBe("cancelled");
  });

  it("lets the cancel outrank what the job's own files would otherwise say", async () => {
    // A SIGKILLed wrapper leaves no exit marker and a log with output in it —
    // the exact fingerprint of a job something else killed mid-run. We killed it,
    // so that is the reason recorded: a filesystem story told over our own cancel
    // would read as a diagnosis of a fault that never happened.
    const tickFile = path.join(tmpBase, "ticks-cancel-reason");
    const started = await startTicker(tickFile, true);
    await waitFor("the job to write output before cancellation", () => fileSize(logPath(started.jobId)) > 0);
    vi.useFakeTimers({ toFake: ["setTimeout"] });
    manager.cancel({ jobId: started.jobId }); // SIGTERM, ignored by the job
    vi.advanceTimersByTime(KILL_ESCALATION_MS + 10); // → SIGKILL, synchronously
    vi.useRealTimers();
    ...
    const settled = expectJob(manager.status({ jobId: started.jobId }));
    expect(fs.existsSync(exitPath(started.jobId))).toBe(false);
    expect(fileSize(logPath(started.jobId))).toBeGreaterThan(0);
    expect(settled.status).toBe("unknown");
    expect(settled.unknownReason).toBe("cancelled");
  });
});
...
describeOnPosix("refusal and payload discipline", () => {
  ...
  it("returns gpu_busy as a structured result instead of throwing", async () => {
    await startJob({ command: LONG_RUNNING, gpuIndex: 0 });
    let result: JobRpcResult | undefined;
    await expect(
      (async () => {
        result = await manager.start({ command: SENTINEL_COMMAND, gpuIndex: 0 });
      })(),
  ...
  // Filling the concurrency ceiling means 32 REAL spawns, and each start also
  // walks every record already on disk — measured at ~10 s on a developer Mac
  // (the same before and after this file's starts became asynchronous), i.e.
  // exactly the default budget. Stated rather than left to luck: the assertion
  // below is about the refusal, never about how fast the box is.
  it("returns too_many_jobs as a structured result instead of throwing", async () => {
    for (let i = 0; i < JOB_MAX_CONCURRENT; i++) {
      await startJob({ command: LONG_RUNNING });
    }
    const refusal = expectRefusal(await manager.start({ command: SENTINEL_COMMAND }));
    expect(refusal.reason).toBe("too_many_jobs");
    expect(JSON.stringify(refusal)).not.toContain(SENTINEL);
  }, 60_000);
  ...
  it("keeps the command out of every reply that did not ask for it", async () => {
    const started = await startJob({ command: SENTINEL_COMMAND, name: "sentinel-job" });
    await waitForExit(started.jobId);
  ...
  it("returns the command only when the caller explicitly asked for it", async () => {
    const started = await startJob({ command: SENTINEL_COMMAND });
    await waitForExit(started.jobId);
...
describeOnPosix("wire shape of a job summary", () => {
  ...
  it("validates for a running job, a cancelled one, and a whole list", async () => {
    const running = await startJob({ command: LONG_RUNNING, gpuIndex: 3 });
    const exited = await startJob({ command: "exit 2" });
...
describeOnPosix("job environment", () => {
  ...
  async function envOf(name: string, req: Partial<Parameters<JobManager["start"]>[0]> = {}) {
    const started = await startJob({ command: `printf %s "$${name}"`, ...req });
    await waitForExit(started.jobId);
  ...
  describeOnPosix("TERM", () => {
    ...
    it("fills TERM in the env a job is spawned with — the launchd/systemd case", async () => {
      ...
      try {
        const started = await startJob({ command: "true" });
        await waitForExit(started.jobId);
    ...
    it("lets a caller-supplied TERM win over the one we fill in", async () => {
      ...
      termFill.envs.length = 0;
      try {
        const started = await startJob({ command: "true", env: { TERM: "xterm-256color" } });
        await waitForExit(started.jobId);
...
// ── What the diagnostic log learns about a job ───────────────────────────────

describeOnPosix("the diagnostic log's view of a job", () => {
  // `job.exit` was declared in the catalogue and never emitted, so the file
  // could not tell a job that is still running from one that is stuck from one
  // that finished — which, for a machine that went silent mid-job, is the
  // question being asked. It hangs off settle(), the single door out of
  // `running`, so every route to a terminal state passes it.
  let logDir: string;

  beforeEach(() => {
    logDir = fs.mkdtempSync(path.join(os.tmpdir(), "aic-jobs-diag-"));
    initDiagLog({ dir: logDir, role: "worker" });
  });
  ...
  /** The log as production writes it: on its own timer, never flushed here. */
  async function diagText(): Promise<string> {
    await new Promise((resolve) => setTimeout(resolve, 20));
    try {
      return fs.readFileSync(path.join(logDir, "worker.log"), "utf8");
    } catch {
      return "";
    }
  }

  it("records job.exit with the id and the code, and nothing else", async () => {
    const started = await startJob({ command: "printf hello; exit 3" });
    await waitForExit(started.jobId);
    // The status poll is what settles the record — the same call any caller makes.
    expect(expectJob(manager.status({ jobId: started.jobId })).status).toBe("exited");

    const text = await diagText();
    expect(text).toContain(`job.exit jobId=${started.jobId}`);
    expect(text).toContain("status=exited");
    expect(text).toContain("exitCode=3");
    // Never the command, never a byte of the job's output.
    expect(text).not.toContain("printf");
    expect(text).not.toContain("hello");
  });

  it("records a job whose process vanished as unknown, not as an exit code", async () => {
    const started = await startJob({ command: LONG_RUNNING });
    process.kill(-(readMetaFile(started.jobId).pid as number), "SIGKILL");
    await waitFor("the killed job to settle", () => {
      return expectJob(manager.status({ jobId: started.jobId })).status !== "running";
    });
