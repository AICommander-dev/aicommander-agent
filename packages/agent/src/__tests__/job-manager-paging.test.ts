...
function startJob(req: Parameters<JobManager["start"]>[0]): JobSummary {
  const job = expectJob(manager.start(req));
...
describe("gpuIndex validation against known GPUs", () => {
  it("refuses an index the machine does not have, naming the cards it does", () => {
    manager.setKnownGpus([device(0, "NVIDIA GeForce RTX 5080")]);

    const refusal = expectRefusal(manager.start({ command: "printf x", gpuIndex: 7 }));
  ...
  it("says so plainly when the machine has no NVIDIA hardware at all", () => {
    manager.setKnownGpus([]);

    const refusal = expectRefusal(manager.start({ command: "printf x", gpuIndex: 3 }));
    expect(refusal.reason).toBe("invalid_request");
    expect(refusal.message).toContain("no NVIDIA GPU");
    expect(refusal.message).toContain("without gpu_index");
  });

  it("accepts an index the machine does have", () => {
    manager.setKnownGpus([device(0, "RTX 5080"), device(1, "RTX 4090")]);

    const job = startJob({ command: "printf x", gpuIndex: 1 });
    expect(job.gpuIndex).toBe(1);
  });

  it("stays permissive while the GPU list is UNKNOWN", () => {
    ...
    expect(startJob({ command: "printf x", gpuIndex: 3 }).gpuIndex).toBe(3);

    manager.setKnownGpus([device(0, "RTX 5080")]);
    manager.setKnownGpus(undefined); // back to "we do not know"
    expect(startJob({ command: "printf x", gpuIndex: 4 }).gpuIndex).toBe(4);
  });

  it("still refuses an index outside the lockfile namespace, list or no list", () => {
    manager.setKnownGpus([device(0, "RTX 5080")]);
    for (const gpuIndex of [-1, 1.5, 4_096]) {
      expect(expectRefusal(manager.start({ command: "printf x", gpuIndex })).reason).toBe(
        "invalid_request",
      );
    }
  });

  it("does not keep the caller's array alive as its own validation input", () => {
    ...
    expect(expectRefusal(manager.start({ command: "printf x", gpuIndex: 1 })).reason).toBe(
      "invalid_request",
    );
  });
});
...
describeOnPosix("job PATH", () => {
  ...
  async function jobPath(env?: Record<string, string>): Promise<string> {
    const job = startJob({ command: 'printf %s "$PATH"', ...(env ? { env } : {}) });
...
it("never leaves a directory behind for a refused gpuIndex", () => {
  manager.setKnownGpus([device(0, "RTX 5080")]);
  expectRefusal(manager.start({ command: "printf x", gpuIndex: 9 }));
