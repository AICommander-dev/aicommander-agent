...
describeOnPosix("the durable record that a cancel was requested", () => {
  ...
  it("does not let the RACE-CLOSING re-read of the exit marker miss a cancel", () => {
    // The interleave the first read's cancellation check never sees: no marker when
    // refresh() first looks, the pid provably gone, and the wrapper's marker on disk
    // by the time the race-closing re-read runs. That window is not rare chance — it
    // is opened by OUR OWN SIGTERM, so it is correlated with cancellation: the shell
    // handles the signal, the EXIT trap writes `$?` = 0, and the re-read used to
    // settle straight to `exited 0`, permanently, over a job the caller cancelled.
    // Post-restart on purpose: the mark is read off meta.json, the in-memory
    // cancelled set of the manager below is empty. That is the half that outlives
    // the agent, and the half the memory set cannot cover.
    const jobId = seedJob({ log: "output before the cancel\n", cancelRequestedAt: Date.now() - 1_000 });
    const exitPath = path.join(jobDir(jobId), "exit");
    // The wrapper completes its write BETWEEN the two reads. The liveness probe is
    // the only thing that happens in between, so it is the hook — no timing, no
    // sleeping: the marker is absent for read one and present for read two.
    const realKill = process.kill.bind(process);
    vi.spyOn(process, "kill").mockImplementation((pid, signal) => {
      if (pid === DEAD_PID) fs.writeFileSync(exitPath, "0");
    ...
    expect(job.unknownReason).toBe("cancelled");
  });
