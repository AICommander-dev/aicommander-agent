...
describe("the Windows operator lookup is inside the protected start window", () => {
  it.each(["cancel", "disconnect"] as const)("does not spawn after %s during lookup", async (action) => {
    ...
    const pending = manager.start({ command: "echo hi", signal: socket.signal, resolveOperator })
      .then((result) => result, (err: unknown) => err);
    await ready;
    const jobId = recordedJobId();
    if (action === "cancel") manager.cancel({ jobId });
    else socket.abort();
    release("o***@example.com");
    const result = await pending;
    expect(resolveOperator).toHaveBeenCalledOnce();
    expect(spawned.count).toBe(0);
    if (action === "cancel") expect(result).toMatchObject({ ok: true, job: { status: "unknown" } });
    else expect(fs.existsSync(path.join(jobsRoot, jobId))).toBe(false);
  });
});
