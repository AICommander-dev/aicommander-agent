...
describe("resolveElevatedAvailability — one word per cause", () => {
  ...
  it("does NOT re-ask every minute about a REFUSAL, which is as durable as an answer", async () => {
    // The complement of the test above, and it is a different question. A `null`
    // from a broken query is a property of this moment; a `null` from a DENIAL is
    // a property of the task's ACL, which cannot change until somebody runs an
    // installer — the very reasoning that earns a real answer ten minutes. Left
    // on the transient TTL it put a PowerShell start, a CIM cold-load and a COM
    // connect on the reconcile path every 60 seconds forever, on every
    // standard-user box with a helper: most of them.
    installHelper();
    helperEnv.endpoints = [{ transport: "unix", path: path.join(tmp, "absent.sock") }];
    const silent = { discover: (async () => ({ ok: false, cause: "unreachable" })) as never };
    vi.useFakeTimers();
    try {
      shell.powershell = "QUERIED=1\r\nADMIN=0\r\nDENIED=1";
      // A refusal is still reported as an UNKNOWN — caching it longer may not
      // turn it into a claim about registration.
      expect(await availability.resolveElevatedAvailability(silent)).toEqual({
        available: false,
        reason: "endpoint_unreachable",
      });
