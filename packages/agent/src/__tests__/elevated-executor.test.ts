...
import { executeElevatedCommand, probeHelperBootId, discoverHelper } from "../elevated-executor.js";
...
describe("executeElevatedCommand — TCP transport (Windows client branch)", () => {
  ...
  it("probeHelperBootId returns the helper's bootId over TCP", async () => {
    const endpoint = await startTcp();
    const bootId = await probeHelperBootId({ endpoint });
    expect(bootId).toBe(helper!.bootId);
  });
});

describe("probeHelperBootId", () => {
  let helper: RunningHelper | undefined;
  let socketPath: string;
  ...
  it("returns null when nothing is listening (dead endpoint)", async () => {
    const endpoint: ElevatedEndpoint = { transport: "unix", path: tempSocketPath() };
    const bootId = await probeHelperBootId({ endpoint, timeoutMs: 500 });
    expect(bootId).toBeNull();
  });

  it("returns null when there is no endpoint on this platform", async () => {
    const bootId = await probeHelperBootId({ endpoint: null });
    expect(bootId).toBeNull();
  });
});

describe("discoverHelper — pool scan + bootId conflict detection", () => {
  ...
  it("returns the shared bootId + the FIRST answering endpoint when all answers agree", async () => {
    ...
    const found = await discoverHelper({ endpoints, attempts: 1 });
    expect(found).not.toBeNull();
    expect(found!.bootId).toBe(helper.bootId);
    expect(found!.endpoint).toEqual(endpoints[0]);
  });
  ...
  it("fails closed (null) when candidates answer with CONFLICTING bootIds", async () => {
  ...
  it("returns null when nothing answers on any candidate after every attempt", async () => {
    const found = await discoverHelper({
