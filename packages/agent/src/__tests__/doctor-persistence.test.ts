...
// ── A PATH THAT WILL NOT ANSWER IS NOT A STALE REGISTRATION ─────────────────
//
// These checks used to stat through a local `stat().catch(() => false)`, which
// folded EACCES, EPERM and EIO into "absent". An autostart entry pointing at a
// perfectly good executable that this process merely may not stat — a scanner
// holding it, a quarantine, an ACL — was therefore reported as pointing at a
// file that "does NOT exist", with the installer as the remedy. The class of
// machine these checks exist for is exactly the class they were misreporting.
describe("paths a scanner or an ACL will not let us stat", () => {
  /**
   * Fail `stat` with `code` for exactly these paths, leaving every other path
   * real. EACCES/EPERM is an ACL or a filter driver, EIO a failing disk; none
   * of them is ENOENT, and none may be reported as one.
   */
  ...
  it("does not call an UNREADABLE Run target and pinned path stale", async () => {
    shell.reg = runKeyDump(`"${realExe}"`);
    shell.powershell = taskDump(realExe);
    const denied = denyStat({ [realExe]: "EACCES" });
    try {
      const results = await persistence.persistenceChecks.run(ctx({ resourcesPath: tmp }));
      const runKey = results.find((r) => r.id === "persistence.run_key")!;
      expect(runKey.verdict).toBe("warn");
      expect(runKey.detail).toMatch(/could not be determined/);
      expect(runKey.detail).toContain("EACCES");
      expect(runKey.detail).not.toMatch(/does NOT exist/);
      expect(runKey.remedy).toMatch(/Inspect it by hand/);
      expect(runKey.remedy).not.toMatch(/no longer exists/);
      // The task's registration is a fact; only the file at the end of its
      // pinned path would not answer, and that is a shortfall of the report
      // rather than the stale-path fault the W7 wording describes.
      const relaunch = results.find((r) => r.id === "persistence.relaunch_task")!;
      expect(relaunch.verdict).toBe("warn");
      expect(relaunch.detail).toMatch(/could not be determined/);
      expect(relaunch.detail).not.toMatch(/does NOT exist|watchdog/);
      expect(relaunch.remedy).toMatch(/Inspect it by hand/);
    } finally {
      denied.mockRestore();
    }
  });

  it("still FAILS both when the OS positively says ENOENT", async () => {
    // The discrimination, from the other side: a genuinely stale path must keep
    // failing, or the fix for the false absence has bought a false pass.
    shell.reg = runKeyDump(`"${realExe}"`);
    shell.powershell = taskDump(realExe);
    const denied = denyStat({ [realExe]: "ENOENT" });
    try {
      const results = await persistence.persistenceChecks.run(ctx({ resourcesPath: tmp }));
      expect(results.find((r) => r.id === "persistence.run_key")!.verdict).toBe("fail");
      expect(results.find((r) => r.id === "persistence.relaunch_task")!.verdict).toBe("fail");
    } finally {
      denied.mockRestore();
    }
  });

  it("reports an EIO on the systemd unit's ExecStart as undetermined, not as a dead service", async () => {
    Object.defineProperty(process, "platform", { value: "linux", configurable: true });
    const node = path.join(tmp, "node");
    const script = path.join(tmp, "agent.js");
    fs.writeFileSync(node, "bin");
    ...
    const denied = denyStat({ [script]: "EIO" });
    try {
      const [result] = await persistence.persistenceChecks.run(ctx());
      expect(result!.id).toBe("persistence.systemd");
      ...
      expect(result!.detail).not.toMatch(/cannot start|all of which exist/);
      expect(result!.facts?.["indeterminate"]).toContain(script);
    } finally {
      denied.mockRestore();
      readFile.mockRestore();
      Object.defineProperty(process, "platform", { value: "win32", configurable: true });
  ...
  it("lets a PROVED absence outrank an undetermined sibling in the same ExecStart", async () => {
    // One path demonstrably gone is enough for the service not to start,
    // whatever the sibling did — so the failure wins, and it names only the
    // path whose absence was actually established.
    Object.defineProperty(process, "platform", { value: "linux", configurable: true });
    const node = path.join(tmp, "node");
    const gone = path.join(tmp, "missing", "agent.js");
    fs.writeFileSync(node, "bin");
    ...
    const denied = denyStat({ [node]: "EACCES" });
    try {
      const [result] = await persistence.persistenceChecks.run(ctx());
      expect(result!.verdict).toBe("fail");
      expect(result!.facts?.["missing"]).toBe(gone);
      expect(result!.facts?.["missing"]).not.toContain(node);
    } finally {
      denied.mockRestore();
      readFile.mockRestore();
      Object.defineProperty(process, "platform", { value: "win32", configurable: true });
  ...
  it("does not tell a Mac its LaunchAgent points at a bundle that has gone", async () => {
    Object.defineProperty(process, "platform", { value: "darwin", configurable: true });
    const plist = path.join(tmp, "dev.aicommander.tray.plist");
    const program = path.join(tmp, "AI Commander");
    fs.writeFileSync(plist, "<plist/>");
    fs.writeFileSync(program, "app");
    shell.launchctl = [`\tpath = ${plist}`, `\tprogram = ${program}`, ""].join("\n");
    const denied = denyStat({ [program]: "EACCES" });
    try {
      const [result] = await persistence.persistenceChecks.run(ctx());
      expect(result!.id).toBe("persistence.launch_agent");
      expect(result!.verdict).toBe("warn");
      expect(result!.detail).toMatch(/could not be determined/);
      expect(result!.detail).toContain("EACCES");
      expect(result!.detail).not.toMatch(/does not exist/);
      expect(result!.remedy).not.toMatch(/Re-install the app/);
