...
describe("build-npm bundle invariants", () => {
  ...
  beforeAll(() => {
    ...
    // cwd MATTERS: esbuild records each inlined source as a comment path
    // relative to the build's working directory, and package-claims.test.ts
    // asserts the `// ../protocol/src/` shape. Inheriting vitest's cwd
    // rebuilt dist/index.js as `// packages/agent/src/` whenever the runner
    // was launched from the repo root, so that suite passed or failed
    // depending on where the command had been typed.
    execFileSync("node", [buildScript], { cwd: pkgRoot, stdio: "ignore" });
