...
export class JobManager {
  ...
  cancel(req: JobCancelRequest): JobRpcResult {
    ...
    if (window) {
      ...
      const ended = this.settle(meta, "unknown", null, Date.now());
   ...
   *
   * Both of them, not just the settled one: `cancelledJobs` used to be left
   * behind by every removal, so an id that never reached settle() — a start
   * cancelled and then erased — accumulated for the life of the process. The
   * two sets exist only to make us report LESS about a record that exists; with
   * the record gone there is nothing left for either to qualify.
   *
