...
/**
 * How long a REFUSAL is reused — and it belongs with the answers, not with the
 * unknowns above.
 *
 * The 60 s TTL is justified by a `null` that "can be true once and false a minute
 * later". A denial is the opposite: Task Scheduler refused because of the task's
 * ACL, which is a structural property of the machine and cannot change until
 * somebody runs an installer — exactly the reasoning that earns a real answer ten
 * minutes. Left on the transient TTL it would put a PowerShell start, a CIM
 * cold-load and a COM connect on the reconcile path every 60 seconds, forever, on
 * every standard-user box with a helper — which is most of them, and precisely
 * the machines this work targets. Cached like an answer, reported like an unknown:
 * what goes in the cache is still `null`, so nothing here becomes a claim about
 * registration.
 */
const REGISTRATION_DENIED_TTL_MS = REGISTRATION_TTL_MS;

let registrationCache: { at: number; registered: boolean | null; ttl: number } | null = null;
...
async function helperRegistered(): Promise<boolean | null> {
  ...
  const now = Date.now();
  if (registrationCache !== null && now - registrationCache.at < registrationCache.ttl) {
    return registrationCache.registered;
  }
  const task = await queryScheduledTask(WIN_HELPER_TASK_NAME);
  const registered = task.queried ? task.registered : null;
  // The TTL is decided by WHY, not by the shape of the value: a refusal is as
  // durable as an answer (REGISTRATION_DENIED_TTL_MS), a broken or absent query is
  // not (REGISTRATION_UNKNOWN_TTL_MS). A `contradiction` belongs with the latter
  // on purpose: the two lookups disagreeing is a transient state worth re-asking
  // about soon, not a settled fact about this account's rights.
  const ttl = task.queried
    ? REGISTRATION_TTL_MS
    : task.cause === "denied"
      ? REGISTRATION_DENIED_TTL_MS
      : REGISTRATION_UNKNOWN_TTL_MS;
  registrationCache = { at: now, registered, ttl };
  return registered;
}
