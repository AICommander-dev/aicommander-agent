...
export async function runSupervisor(opts: SupervisorOptions = {}): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    ...
    const start = () => {
      ...
      diag("supervisor.worker_spawned", { spawn: spawnCount, pid: child.pid ?? null });
      ...
      child.on("exit", (code, signal) => {
        ...
        diag("supervisor.worker_exit", { code: code ?? null, signal: signal ?? null });
    ...
    watchdog = setInterval(() => {
      ...
      if (Date.now() - last > WATCHDOG_STALL_MS) {
        // A wedged event loop, which from outside is indistinguishable from a
        // healthy idle agent — the exact ambiguity this log exists to remove.
        diag("supervisor.worker_stalled", { staleMs: Date.now() - last });
        forceRestart();
      }
    }, WATCHDOG_CHECK_INTERVAL_MS);
    watchdog.unref?.();
