...
async function checkRegistered(): Promise<CheckResult> {
  ...
  if (process.platform === "win32") {
    ...
    if (!task.queried) {
      ...
      const because =
        task.cause === "denied"
          ? "Task Scheduler refused the read, which it only does for a task that EXISTS — so this is not a " +
            "missing helper. Re-run the doctor from an elevated prompt to read its state."
          : // NOT the "nothing was learned" sentence, and that is the point of the
            // separate cause. Here the Task Scheduler service DID read the task,
            // so saying nothing was learned would contradict the reason printed
            // one clause earlier. What it does not do is settle registration: two
            // lookups disagreeing is not a positive answer, so this is neither a
            // missing helper nor a present one.
            ...
            ? "Something WAS learned — the Task Scheduler service read the task — but the two lookups disagree, " +
              ...
              "installer registers."
            : "Registration here is UNKNOWN, not missing: nothing was learned about the task either way. " +
              "Re-running the doctor from an elevated prompt may help if the lookup was refused, but a query " +
              "that never ran or never returned will need whatever stopped it fixed first.";
      return skipped(
        id,
        title,
