 ...
 */
async function sha256File(absolute: string, io: AsarBlindFs): Promise<string> {
...
export async function checkManifest(ctx: DoctorContext, fsEnv?: AsarFsEnv): Promise<ManifestVerdict> {
  const id = "install.manifest";
  const title = "Install manifest";
  // The unpatched `fs`, threaded through every syscall this check makes: the
  // manifest records what is on DISK, and inside Electron the ordinary `fs`
  // answers about a virtual filesystem in which app.asar is an empty directory
  // (asar-fs.ts). `fsEnv` is the test seam that lets a test be Electron.
  const io = fsEnv ? selectAsarBlindFs(fsEnv) : asarBlindFs();
  ...
  try {
    loaded = await loadInstallManifest(resourcesPath, io);
  } catch (err) {
    return {
      root: null,
  ...
  let realRoot: string;
  try {
    realRoot = path.resolve(await io.realpath(root));
  } catch (err) {
    return {
      root,
      result: skipped(
  ...
  for (const { entry, absolute: spelled } of targets) {
    const resolved = await manifestEntryRealTarget(realRoot, spelled, dirs, io);
    ...
    // `io` is what keeps app.asar out of this branch: to Electron's patched
    // `fs` the archive IS a directory, and a healthy Mac was told a shipped
    // file "could not be read" on every doctor run (asar-fs.ts).
    if (!stats.isFile()) {
      unreadable.push(entry.path);
    ...
    try {
      const actual = await sha256File(absolute, io);
      hashed++;
