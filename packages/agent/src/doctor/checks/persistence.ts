...
/**
 * Can a NON-ADMINISTRATOR see this task at all?
 *
 ...
 *
 * So "there is no such task" is EVIDENCE for the Update task from any account,
 * and only from an elevated one for the Relaunch task: a standard-user tray is
 * told the Relaunch task does not exist whether or not it does. Mirrored rather
 * than imported because the installer is PowerShell in another package; if those
 * SDDLs change, the worst this produces is a warning where a failure was due.
 */
const READABLE_BY_USERS: Record<string, boolean> = {
  [UPDATE_TASK_NAME]: true,
  [RELAUNCH_TASK_NAME]: false,
};
...
async function windowsPersistence(ctx: DoctorContext): Promise<CheckResult[]> {
  ...
  for (const [taskName, id, title] of [
    ...
    if (!task.queried) {
      // The query never ran (no PowerShell, a policy block, a timeout). That is
      // a statement about the diagnostic, not about the machine.
      results.push(
        skipped(
          id,
          title,
    ...
    if (!task.registered) {
      ...
      // An absent task the asking account could not have READ anyway is not
      // evidence of anything. The tray runs unelevated on most machines.
      const visible = READABLE_BY_USERS[taskName] === true || task.elevated === true;
      results.push(
        visible
          ? fail(
              id,
          ...
          : warn(
              id,
              title,
              `the scheduled task "${taskName}" did not come back from Windows. It is either not registered — ` +
                `in which case nothing ${consequence} — or simply not visible to this process, because its ` +
                "ACL grants Administrators only and this run is not elevated.",
              "Re-run this command from an elevated prompt to tell the two apart. If the app does not start " +
                "at boot, re-run the installer: it registers the task from its own elevated context.",
              { ...baseFacts, elevated: task.elevated },
            ),
      );
      continue;
