        ...
        results.push(
          packaged
            ? warn(
                id,
                title,
                `the scheduled task "${taskName}" IS registered — Task Scheduler refused this process the right ` +
                  "to read it, which it only does for a task that exists — but its action could not be read, so " +
