...
/**
 * Does the path an autostart entry names still exist?
 *
 * TRI-STATE, through the directory's shared `pathPresence`, and not the
 * `stat().catch(() => false)` this file used to keep. That one-liner folded
 * EACCES, EPERM and EIO into "absent", so a unit file or a Run value pointing at
 * an executable this process merely may not stat was reported as a STALE
 * REGISTRATION — "which does NOT exist", "the service cannot start" — with
 * "re-run the installer" as the remedy. The one class of machine this check
 * exists for is the one whose files a scanner or an ACL is holding, and that is
 * exactly the machine it was misreporting. presence.ts owns the rule now, so
 * this file and priv-helper.ts cannot drift apart from it again.
 *
 * Still ASYNCHRONOUS, like every filesystem call in this directory: the tray
 * runs these checks on Electron's main loop and a synchronous stat on a path a
 * scanner is holding blocks the relay heartbeat.
 */
type Existence = "present" | "absent" | "unknown";

async function existence(target: string): Promise<{ state: Existence; error: string | null }> {
  const presence = await pathPresence(target);
  return presence.kind === "unknown"
    ? { state: "unknown", error: presence.error }
    : { state: presence.kind === "present" ? "present" : "absent", error: null };
}

/** The one sentence an undetermined path gets, so no branch invents its own. */
const UNREADABLE_REMEDY =
  "Nothing here says the entry is stale — the path answered neither way, which is how a scanner holding a " +
  "file, a quarantine, and a directory this account may not traverse all look. Inspect it by hand (elevated, " +
  "on Windows) before re-running the installer: a re-install does not address any of those.";
...
async function windowsPersistence(ctx: DoctorContext): Promise<CheckResult[]> {
      ...
      if (found.state === "present") {
        ...
        results.push(
          warn(
            id,
            "Autostart (Run key)",
            `the Run value points at ${target}, and whether it still exists could not be determined: ` +
              `${found.error}.`,
            UNREADABLE_REMEDY,
            facts,
          ),
  ...
  for (const [taskName, id, title] of [
    ...
    const pinnedPresence = await existence(pinned);
    results.push(
      pinnedPresence.state === "present"
        ? ok(id, title, `"${taskName}" is registered and pinned to ${pinned}, which exists.`, facts)
        : pinnedPresence.state === "unknown"
          ? // The task IS registered and its pinned path IS readable as a string —
            // only the file at the end of it would not answer. That is a
            // shortfall of the report, not the W7 stale-path fault, and calling
            // it stale sends the reader to an installer run for a file that may
            // be sitting right there under a scanner's lock.
            warn(
              id,
              title,
              `"${taskName}" is registered and pinned to ${pinned}, but whether that path exists could not be ` +
                `determined: ${pinnedPresence.error}.`,
              UNREADABLE_REMEDY,
              facts,
            )
          : fail(
...
async function linuxPersistence(): Promise<CheckResult[]> {
  ...
  /** Paths whose absence was NOT proved — never counted towards `missing`. */
  const indeterminate: string[] = [];
  for (const target of targets) {
    const found = await existence(target);
    if (found.state === "absent") missing.push(target);
    else if (found.state === "unknown") indeterminate.push(`${target} (${found.error})`);
  }
  ...
  // A PROVED ABSENCE OUTRANKS AN UNDETERMINED ONE: one path of the ExecStart
  // demonstrably gone is enough for the service not to start, whatever the
  // sibling path did. With nothing proved gone, an undetermined path is the only
  // thing left to say — "all of which exist" would be a claim about a file this
  // run never managed to look at.
  return [
    missing.length > 0
      ? fail(
          id,
      ...
      : indeterminate.length > 0
        ? warn(
            id,
            title,
            `whether the unit's ExecStart still points at files that exist could not be determined: ` +
              `${indeterminate.join("; ")}.`,
            UNREADABLE_REMEDY,
            { ...facts, indeterminate: indeterminate.join("; ") },
          )
...
async function macPersistence(): Promise<CheckResult[]> {
  ...
  const missing: string[] = [];
  /** Named by launchd, but not stat-able from here — never counted as missing. */
  const indeterminate: string[] = [];
  ...
  // A PROVED ABSENCE OUTRANKS AN UNDETERMINED ONE, and the reverse of that is
  // the combination worth naming: the plist read fine and the program did not,
  // which is neither "the files it names exist" nor "the bundle has moved". The
  // registration itself is a fact launchd gave us either way, so this stays a
  // warn about the report's shortfall rather than a failure about the machine.
  return [
    missing.length > 0
      ? fail(
          id,
      ...
      : indeterminate.length > 0
        ? warn(
            id,
            title,
            `the "${MAC_AGENT_LABEL}" LaunchAgent is registered, but whether the files it names exist could not ` +
              `be determined: ${indeterminate.join("; ")}.`,
            UNREADABLE_REMEDY,
            { ...facts, indeterminate: indeterminate.join("; ") },
          )
        : ok(
