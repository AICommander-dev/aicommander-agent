...
import { asarBlindFs, type AsarBlindFs } from "./asar-fs.js";
 ...
 *
 * `io` is the unpatched `fs` (asar-fs.ts): inside Electron the ordinary one
 * answers about a virtual filesystem, and this pass exists to describe the disk.
 */
export async function manifestEntryRealTarget(
  ...
  if (!resolved) {
    resolved = await resolveDir(realRoot, dir, io);
...
async function resolveDir(realRoot: string, dir: string, io: AsarBlindFs): Promise<ManifestEntryRealTarget> {
  let real: string;
  try {
    real = await io.realpath(dir);
  } catch (err) {
