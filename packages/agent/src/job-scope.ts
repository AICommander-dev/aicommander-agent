 ...
 * five seconds, reached lazily from spawnJob — i.e. from inside what was then a
 * fully synchronous JobManager.start(), i.e. on the WS frame handler's own
 * thread. (start() is asynchronous today, for a related reason of its own — the
 * Windows job scripts — but it still never awaits anything from HERE.) On exactly the
 ...
 * calls. JobManager.start() is asynchronous (it awaits the Windows job scripts;
 * see job-scripts.ts), but this call is not one of the things it awaits: it is a
 * plain read of an already-settled memo, and it must stay that way, because
 * every await on the start path is a window some other frame can arrive in.
 *
